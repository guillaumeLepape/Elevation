    ...
    if (information_ && !noRule_) {
      ...
      while (!out) {
        int resultInformation =
            Selection::select({&informationWeaponInventory, &informationCombo,
                               &noInformation, &noInformationAnymore},
...
Plug* Fight::choosePlug() {
  ...
  int resultChoosePlug =
      Selection::select(choosePlugActions, data::Action::titleChoosePlug);
...
const ChooseWeaponResult Fight::chooseWeapon(Plug* const choosenPlug) {
  ...
  int resultUseWeapon =
      Selection::select(useWeapons, data::Action::titleChooseWeapon);
