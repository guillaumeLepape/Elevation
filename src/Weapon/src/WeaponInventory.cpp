...
bool WeaponInventory::addWeapon(const Weapon* weapon) {
  auto it =
      std::find_if(std::list<std::unique_ptr<const Weapon>>::cbegin(),
  ...
  // If this condition is true, weapon is not in weapons_ so add it and sort the
  // weapons_ vector
  if (!present) {
    std::list<std::unique_ptr<const Weapon>>::emplace_back(weapon);
...
bool WeaponInventory::containWeaponType(const WeaponType& weaponType) const {
  for (auto w = std::list<std::unique_ptr<const Weapon>>::cbegin();
       w != std::list<std::unique_ptr<const Weapon>>::cend(); w++) {
