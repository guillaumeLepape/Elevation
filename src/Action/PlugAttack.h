...
class PlugAttack : public Action {
 ...
 public:
  PlugAttack(Player* const player, Plug* const plug,
             const std::string& statement, const std::string& result)
      : Action(statement, result), player_(player), plug_(plug) {}
  ...
  void triggerAction() override {
    ...
    if (!(plug_->dead()) &&
      ...
      actionWriter_.updateResult(data::Action::resultPlugAttack(
